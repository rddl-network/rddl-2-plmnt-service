...
func (r2p *R2PService) ExecutePotentialConversion(conversion ConversionRequest) (deleteEntry bool, err error) {
	...
	if err != nil {
		...
		r2p.logger.Error(msg)
		err = errors.New(msg)
		return
	}
	if len(txDetails) == 0 {
		msg := "the address hasn't received any transactions for the given asset: %s - %s"
		r2p.logger.Debug(msg, conversion.ConfidentialAddress, cfg.AcceptedAsset)
		return
	...
	r2p.logger.Info("Conversion: " + conversion.ConfidentialAddress + " received tx: " + txDetails[0].TxIDs[0])
...
func (r2p *R2PService) checkMintRequest(liquidTxHash string) (code int, err error) {
	...
	if err != nil {
		r2p.logger.Error("error while fetching mint request: %w", err)
	...
	if mr != nil {
		r2p.logger.Info("error while fetching mint request: %w", err)
